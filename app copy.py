...
def display_file_info_1(data):
    ...
    try:
        data = {
                ...
                "Participant": safe_get(info, ['subject_info', 'id']),
                "Digitized points": "Not available",
                "Good channels": f"{len(info.get('chs', [])) - len(mne.pick_types(raw.info, eeg=False, stim=True))} EEG, {len(mne.pick_types(raw.info, stim=True))} Stimulus",
                ...
                "Filenames": uploaded_file if uploaded_file else "Unknown",
                "Duration": f"{raw.times[-1] / 60:.2f} minutes" if raw.times else "Unknown"
            }
    ...
    with st.expander("File Information", expanded=True):
        try:
            ...
            file_data = {
                ...
                "Digitized points": safe_get(info, ['dig'], "Not available"),
